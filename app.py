...
print("DEBUG: Initializing SentenceTransformer model (this may download model on first run)...")
...
def evaluate_resume():
    ...
    evaluation_result = None
    recommended_jobs = [] # ADDED: Initialize empty list for recommendations
    ...
    if request.method == 'POST':
        ...
        if resume_file.filename == '':
            flash('No selected resume file', 'error')
            return render_template('evaluate_resume.html', uploaded_data=uploaded_data, evaluation_result=evaluation_result, recommended_jobs=recommended_jobs, current_user=current_user)
        ...
        if resume_file and allowed_file(resume_file.filename):
            ...
            # --- ADDED: Job Recommendation Logic ---
            try:
                # Use the full extracted resume text for embedding
                resume_embedding = embedding_model.encode(resume_text, convert_to_tensor=True)
                
                all_jobs_in_db = Job.query.all()
                job_recommendations_raw = []

                for job_db_obj in all_jobs_in_db:
                    if job_db_obj.job_description:
                        job_description_embedding = embedding_model.encode(job_db_obj.job_description, convert_to_tensor=True)
                        # Calculate cosine similarity
                        cosine_scores = util.pytorch_cos_sim(resume_embedding, job_description_embedding)
                        score = cosine_scores.item() # Get the float value
                        
                        job_recommendations_raw.append({
                            'job_id': job_db_obj.id,
                            'job_title': job_db_obj.job_title,
                            'company_name': job_db_obj.company_name,
                            'score': score
                        })
                
                # Sort by score in descending order and get top N (e.g., top 5)
                recommended_jobs = sorted(job_recommendations_raw, key=lambda x: x['score'], reverse=True)[:5]
                print(f"DEBUG: Generated {len(recommended_jobs)} job recommendations.")

            except Exception as e:
                print(f"ERROR: Failed to generate job recommendations: {e}")
                flash('Failed to generate job recommendations at this time.', 'warning')
                recommended_jobs = [] # Ensure it's empty if an error occurs
            # --- END ADDED Job Recommendation Logic ---

            # Store results in temporary server-side cache
            analysis_id = str(uuid.uuid4())
            temp_analysis_cache[analysis_id] = {
                'uploaded_data': uploaded_data,
                'evaluation_result': evaluation_result,
                'recommended_jobs': recommended_jobs # ADDED: Store recommendations in cache
            }
            session['pending_analysis_id'] = analysis_id
            ...
            if not current_user.is_authenticated:
                ...
                temp_analysis_cache.pop(analysis_id, None)
                session.pop('pending_analysis_id', None)
                # Pass recommendations directly if already logged in
                return render_template('evaluate_resume.html', evaluation_result=evaluation_result, uploaded_data=uploaded_data, recommended_jobs=recommended_jobs, current_user=current_user)
        else:
    ...
    if pending_analysis_id and pending_analysis_id in temp_analysis_cache:
        ...
        uploaded_data = cached_data['uploaded_data']
        evaluation_result = cached_data['evaluation_result']
        recommended_jobs = cached_data.get('recommended_jobs', []) # ADDED: Retrieve recommendations from cache
        flash('Welcome back! Here are your previous analysis results.', 'info')
...
# --- This block MUST be at the very end of your app.py file ---
if __name__ == '__main__':
    # This conditional ensures that heavy, one-time setup code runs only once,
    # even when Flask's auto-reloader (in debug mode) causes the script to run twice.
    if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        print("DEBUG: Running one-time app setup (DB, Sheets, LLM models loading)...")
        with app.app_context():
            db.create_all() # This creates tables, which relies on models being defined globally
            
            # REMOVED: load_jobs() # Job data is now in the database
        print("DEBUG: One-time app setup complete.")
    else:
